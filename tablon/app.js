...
import { demoTasks, childAction, adultValidate, adultUndo } from './domain.mjs';
...
function renderTask(task, childView = false) {
  ...
  const statusText = { pending: 'Pendiente', child_done: 'Hecha por niño', validated: 'Validada', adult_done: 'Hecha por adulto' }[status] || 'Estado no reconocido';
  let actions = '';
...
$('#sign-in').addEventListener('click', () => { if (runtime.localDevelopment) { const role = prompt('Solo demo local: escribe adulto o infantil. No verifica identidad.'); const map = { adulto: 'adult', infantil: 'child' }; if (map[role?.toLowerCase()]) enterLocal(map[role.toLowerCase()]); else toast('Acceso local cancelado.'); } else toast('Inicio de sesión pendiente: no hay autenticación conectada.'); });
$('#sign-out').addEventListener('click', () => { state.user = null; show($('#app'), false); $('#sign-out').classList.add('hidden'); $('#sign-in').classList.remove('hidden'); $('#mode-badge').textContent = runtime.localDevelopment ? 'Desarrollo local · no seguro' : 'Sin sesión'; });

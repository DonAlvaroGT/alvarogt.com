...
class GoClientAuthHtmlTests(unittest.TestCase):
    ...
    def test_hoy_semana_switch_inside_go(self):
        ...
        self.assertEqual(self.html.count('data-vista="'), 2)
        self.assertIn("go.vista", Path(__file__).parent.joinpath("vista.mjs").read_text(encoding="utf-8"))
        tiempo = Path(__file__).parent.joinpath("tiempo.mjs").read_text(encoding="utf-8")
        self.assertIn("forecast_days=7", tiempo)

...
class CasaShellTests(unittest.TestCase):
    ...
    def test_login_on_parent_not_iframe(self):
        ...
        self.assertIn("#scroller iframe", HTML)
    ...
    def test_lazy_iframes_overlay_resume(self):
        ...
        self.assertIn("display = on ? 'block' : 'none'", JS)
        self.assertIn("paintFrame", JS)
        ...
        self.assertIn("data-trip-soon", JS)
        self.assertIn("has-trip", JS)
        self.assertIn("refreshViajesTripDot", JS)
        self.assertIn("applyViajesHoy", JS)
        self.assertIn("data-viajes-vista", JS)
        self.assertIn("sports-star-5", JS)
        self.assertIn("refreshGoHotDot", JS)
        self.assertIn("has-hot", JS)
        self.assertIn('iframe[data-casa-view="go"]', HTML)
        self.assertIn("visibility: hidden", HTML)
        self.assertNotIn("setAppBadge", JS)
        self.assertNotIn("pushManager", JS)
        self.assertEqual(HTML.count('class="tab-dot"'), 3)
